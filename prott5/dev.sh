...
case $1 in
    ...
    --test|-t)
        docker run --rm $DOCKER_IMAGE:$VERSION \
            python -c "import torch, transformers, sentencepiece; from transformers import T5Tokenizer, T5EncoderModel; print('torch', torch.__version__); print('transformers', transformers.__version__)"
    ;;
    *)
