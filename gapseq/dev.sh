...
case $1 in
    ...
    -rd) # docker
            # -e XDG_CACHE_HOME="/ws"\
            # --mount type=bind,source="$HERE/src",target="/app"\
        shift
        mkdir -p ./scratch/docker
        docker run -it --rm \
            -u $(id -u):$(id -g) \
            --mount type=bind,source="$HERE/scratch/docker",target="/ws"\
            --mount type=bind,source="$HERE/lib/gapseq",target="/opt/conda/envs/external_gapseq_env/share/gapseq"\
            --workdir="/ws" \
