...
case $1 in
    ...
    -bd) # docker
        ...
        ! [ -f diamond ] && wget https://github.com/bbuchfink/diamond/releases/download/v2.1.13/diamond-linux64.tar.gz \
            && pigz -dc diamond-linux64.tar.gz | tar -xf - \
            && rm diamond-linux64.tar.gz
        ! [ -e hmmer-3.3.1 ] && wget http://eddylab.org/software/hmmer/hmmer-3.3.1.tar.gz \
            && pigz -dc hmmer-3.3.1.tar.gz | tar -xf - \
            && rm hmmer-3.3.1.tar.gz
        cd $HERE
    ...
    -rd) # docker
            # -e XDG_CACHE_HOME="/ws"\
        shift
        mkdir -p ./scratch/docker/d
        docker run -it --rm \
            -u $(id -u):$(id -g) \
            --mount type=bind,source="$HERE/scratch/docker",target="/ws"\
            --mount type=bind,source="$HERE/scratch/docker/d",target="/app/PaperBLAST/tmp/downloaded"\
            --workdir="/ws" \
