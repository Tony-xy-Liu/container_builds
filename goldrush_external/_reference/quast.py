...
def MakeJobs():
    return [
        dict(
            name=f"test_goldrush",
            asm="/home/rtcai12/project/goldrush/out/goldrush_test_golden_path.goldpolish-polished.span2.dist500.tigmint.fa.k40.w250.ntLink-5rounds.fa",
            reads=[
                "/home/rtcai12/project/goldrush/test_reads.fq",
            ],
        ),
    ]
...
def RunJob(DATA, OUT_DIR, job_num, cpus, mem, log):
    ...
    load_singularity = f"module load gcc/9.4.0 apptainer/1.2.2"
    ...
    with open(bounce, "w") as f:
        ...
        f.write(f"""\
            ...
            minimap2 \
                -a -2 -t {cpus-1} --sam-hit-only --secondary=no \
                -x map-ont \
