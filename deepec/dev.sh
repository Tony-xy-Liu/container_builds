#!/bin/bash
# version 1.1 
# this is a general dev script
# not all functionality will make sense in this context

HERE=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
echo $HERE
DEV_USER=hallamlab
# VER="$(cat $HERE/version.txt).$(git branch --show-current)-$(git rev-parse --short HEAD)"
NAME=external_deepec
VER="0.4.1"
...
case $1 in
    ...
    -bd) # docker
        ...
        ! [ -e tini ] && wget https://github.com/krallin/tini/releases/download/${TINI_VERSION}/tini
        ! [ -e tini ] && git clone https://bitbucket.org/kaistsystemsbiology/deepec.git
        cd $HERE
    ...
    -rd) # docker
        ...
        docker run -it --rm \
            ...
            --mount type=bind,source="$HERE/../../data",target="/data"\
            --workdir="/ws" \
