...
def predict_ec(ref_db_file, target_fasta_file, output_file, output_dir, threads):
    uniprot_ec_info = {}
    pfam_uniprot_id_info = {}

    blastp_result = output_dir+'/blastp_result_temp.txt'
    run_blastp(target_fasta_file, blastp_result, ref_db_file, threads)
    try:
...
def run_blastp(target_fasta, blastp_result, db_dir, threads):
